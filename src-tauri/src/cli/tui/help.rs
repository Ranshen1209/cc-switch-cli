...
fn help_for_target(target: HelpTarget, app: &App, data: &UiData) -> HelpContent {
    match target {
        ...
        HelpTarget::Sessions => HelpContent::new(
            ...
            help_lines(
                "会话始终只显示当前应用，结果由项目范围 × / 搜索共同决定。\n←/→ 切换列表和详情，h/l 是备用键；↑/↓ 逐项移动，PgUp/PgDn 按页移动。p 打开项目选择器；Home/End 在当前会话列表或消息历史中跳到首尾，Shift+←/→ 查看完整目录，Shift+Home/End 直达目录两端。\n详情中的消息按需分页，覆盖完整逻辑历史；为保持响应速度，超长单条正文只显示有界预览。详情内的 / 只过滤当前消息页，保留筛选时仍可用 PgUp/PgDn/Home/End 浏览其他历史页。\nClaude、Codex、Gemini 和 OpenCode 的费用与 token 是当前仍保留在 proxy_request_logs 中、经有效过滤去重后能以确定性 ID 归属到该会话的明细小计；Hermes 则直接显示其 state.db 提供的源估算。它们不是端到端账单，也不是历史上曾记录的全部金额。无前缀只表示“当前 session ID 的已记录小计完整”；≥ 表示可证明的下界，~ 表示 Hermes 源提供的估算，- 表示没有可显示的数据。Codex 根会话费用不含独立 subagent 线程的费用。\n即使显示为完整，数据源仍有上限：Gemini 插入失败、Claude/Codex malformed 行、无法归属的代理 Generated ID、同 mtime 内容替换、尚未触发重查的实时代理日志及 SQLite REAL 求和精度，都可能使它达不到帐单级完整。\n“未知目录”位于项目列表末尾，只包含缺少项目目录的旧会话；精确项目按词法规范化后的完整目录匹配。",
                "Sessions always show the current app; results combine Project scope × / Search.\nUse ←/→ to switch between the list and details; h/l are aliases. Use ↑/↓ to move one item and PgUp/PgDn to move by a page. Press p to choose a project; Home/End jumps to either end of the active session list or message history, Shift+←/→ reveals the complete directory, and Shift+Home/End jumps to either path end.\nDetail messages are paged on demand across the complete logical history. To stay responsive, an unusually long individual body is shown as a bounded preview. In details, / filters the current message page only; PgUp/PgDn/Home/End still browse other history pages while the filter is retained.\nFor Claude, Codex, Gemini, and OpenCode, Cost and tokens are the retained attributable detail subtotal: rows still present in proxy_request_logs after the effective deduplication filter and deterministically mapped to this session. Hermes instead displays source estimates directly from its state.db. These values are neither an end-to-end bill nor every amount ever recorded. No prefix means only that the recorded subtotal for the current session ID is complete; ≥ marks a provable lower bound, ~ marks an estimate supplied by Hermes, and - means no displayable data. A Codex root session excludes costs from independent subagent threads.\nEven Complete has source-level limits: failed Gemini inserts, skipped malformed Claude/Codex rows, unattributable proxy Generated IDs, same-mtime content replacement, live proxy rows before a requery, and SQLite REAL summation can all prevent billing-grade completeness.\nUnknown directory is last and contains only legacy sessions without a project directory; exact projects match the complete lexically normalized directory.",
            ),
        ),
